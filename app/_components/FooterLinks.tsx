      ...
      {links.map(({ link, name, external }, i) => {
        ...
        return (
          <Comp
            ...
            {...targetProps}
            href={lowercase(link)}
            className="fit transition-all duration-300 ease-linear link-underline after:scale-0 after:transition-all after:duration-300 after:ease-linear hover:after:scale-100 hover:text-red-500 dark:after:bg-white dark:hover:text-gray-400"
            >
            <p>{name}</p>
          </Comp>
        );
